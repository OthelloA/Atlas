...
pub async fn analyze_repo_impl(
    ...
    emit_progress(app, 1, "Fetching repository tree", FetchStatus::Running, Some(repo_name.clone()), None);
    let github = GithubClient::new(resolve_github_token(settings));
    let tree = github.get_repo_tree(&parsed.owner, &parsed.repo).await?;
    if tree.iter().any(|f| f.path == ".atlas-tree-truncated" || f.path == ".repo-lens-tree-truncated") {
        warnings.push("GitHub reported the repository tree as truncated".to_string());
    }
    emit_progress(app, 1, "Fetching repository tree", FetchStatus::Done, Some(format!("{} files", tree.len())), None);

    emit_progress(app, 2, "Detecting monorepo and selecting files", FetchStatus::Running, None, None);
    ...
    let root_contents = github
        .get_file_contents_batch(&parsed.owner, &parsed.repo, &root_manifest_files, MAX_CONTENT_CONCURRENCY, MAX_BYTES_PER_FILE)
        .await;
    ...
    let (selected_files, truncated) = select_analysis_files(&tree, selected_scope.as_deref(), MAX_ANALYSIS_FILES, &mut warnings);
    emit_progress(app, 2, "Detecting monorepo and selecting files", FetchStatus::Done, Some(format!("{} selected", selected_files.len())), None);

    emit_progress(app, 3, "Reading selected files", FetchStatus::Running, None, Some((0, selected_files.len() as u32)));
    let contents = github
        .get_file_contents_batch(&parsed.owner, &parsed.repo, &selected_files, MAX_CONTENT_CONCURRENCY, MAX_BYTES_PER_FILE)
        .await;
    ...
    emit_progress(app, 3, "Reading selected files", FetchStatus::Done, Some(format!("{} files", contents.len())), Some((contents.len() as u32, selected_files.len() as u32)));
    ...
    emit_progress(app, 4, "Detecting stack", FetchStatus::Done, Some(if stack.is_empty() { "unknown stack".to_string() } else { stack.join(" · ") }), None);

    emit_progress(app, 5, "Extracting symbols and call trace", FetchStatus::Running, None, None);
    ...
    let (call_trace, trace_warnings) = build_call_trace(&entry_points, &symbols, MAX_CALL_TRACE_DEPTH);
    warnings.extend(trace_warnings);
    emit_progress(app, 5, "Extracting symbols and call trace", FetchStatus::Done, Some(format!("{} symbols", symbols.len())), None);

    emit_progress(app, 6, "Writing overview", FetchStatus::Running, None, None);
    let overview_md = if settings.model.trim().is_empty() {
        let warning = "No model configured; deterministic analysis completed without LLM overview".to_string();
        warnings.push(warning.clone());
        format!("# {}\n\n{}\n\nStack: {}\n", repo_name, warning, if stack.is_empty() { "unknown".to_string() } else { stack.join(", ") })
    } else {
        match AiBackend::new(&settings.model, &settings.aws_profile).await {
            Ok(ai) => {
                let prompt = build_repo_overview_prompt(&repo_name, &stack, &entry_points, &symbols, &call_trace, &contents, &warnings, Some(&monorepo));
    ...
    emit_progress(app, 7, "Building onboarding pack", FetchStatus::Running, None, None);
    ...
    emit_progress(app, 7, "Building onboarding pack", FetchStatus::Done, None, None);
