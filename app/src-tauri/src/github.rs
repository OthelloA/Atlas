...
use crate::types::{CommentAuthor, FileContent, PrFile, PrMetadata, RepoFile, ReviewComment, ReviewRequestItem, ReviewThread};
use base64::Engine;
...
impl GithubClient {
    ...
    pub async fn get_default_branch(&self, owner: &str, repo: &str) -> Result<String, String> {
        ...
        let resp = self.send_checked(&url, "application/vnd.github.v3+json").await?;
        let metadata: RepoMetadataResponse = resp
            .json()
            .await
            .map_err(|e| format!("Failed to parse repository metadata: {}", e))?;
        Ok(metadata.default_branch)
    }

    pub async fn get_repo_tree(&self, owner: &str, repo: &str) -> Result<Vec<RepoFile>, String> {
        ...
        let branch_resp = self.send_checked(&branch_url, "application/vnd.github.v3+json").await?;
        let branch: BranchResponse = branch_resp
            .json()
            .await
            .map_err(|e| format!("Failed to parse branch metadata: {}", e))?;

        let tree_url = format!(
            "https://api.github.com/repos/{}/{}/git/trees/{}?recursive=1",
            owner, repo, branch.commit.sha
        );
        let tree_resp = self.send_checked(&tree_url, "application/vnd.github.v3+json").await?;
        let tree: GitTreeResponse = tree_resp
            .json()
            .await
            .map_err(|e| format!("Failed to parse repository tree: {}", e))?;

        let mut files: Vec<RepoFile> = tree
            .tree
            .into_iter()
        ...
        if tree.truncated.unwrap_or(false) {
            files.push(RepoFile {
                path: ".atlas-tree-truncated".to_string(),
                file_type: "warning".to_string(),
                size: None,
                sha: None,
            });
        }
    ...
    pub async fn get_file_contents_batch(
        &self,
        owner: &str,
        repo: &str,
        files: &[RepoFile],
        ...
        let branch = self.get_default_branch(owner, repo).await.unwrap_or_else(|_| "HEAD".to_string());
            ...
            .map(|file| {
                ...
                async move {
                    if let Some(size) = file.size {
                        if size as usize > max_bytes_per_file {
                            return FileContent {
                                path: file.path,
                                content: String::new(),
                                truncated: true,
                                warning: Some(format!("Skipped file larger than {} bytes", max_bytes_per_file)),
                            };
                        }
                    }

                    match self.get_file_content(&owner, &repo, &file.path, &branch).await {
                        Ok(content) => {
                            if content.len() > max_bytes_per_file {
                                FileContent {
                                    path: file.path,
                                    content: content.chars().take(max_bytes_per_file).collect(),
                                    truncated: true,
                                    warning: Some(format!("Truncated file to {} bytes", max_bytes_per_file)),
                                }
                            } else {
                                FileContent { path: file.path, content, truncated: false, warning: None }
                            }
                        }
                        Err(err) => FileContent {
                            path: file.path,
